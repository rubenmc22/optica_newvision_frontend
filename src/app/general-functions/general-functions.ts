...
export class GeneralFunctions {
  ...
  isValidPhone(telefono: string): boolean {
    return /^[0-9]{11,12}$/.test(telefono);
  }
  ...
  // Función para evitar que el usuario ingrese caracteres no numéricos
  validarSoloNumeros(value: string): string {
    return value.replace(/\D/g, ''); // Elimina cualquier carácter que no sea número
  }
